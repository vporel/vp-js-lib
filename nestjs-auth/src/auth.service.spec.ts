...
describe("AuthService", () => {
	...
	describe("signIn", () => {
		...
		it("should throw UnauthorizedException if the password is incorrect", async () => {
			userFinder.findByEmail = jest
				.fn()
				.mockResolvedValue({ user: { email: testEmail }, userClass: "Professional" });
	...
	describe("getEmailFromAuthMethod", () => {
		...
		it("should call third-party auth service if method is not email", async () => {
			...
			const email = await service.getEmailFromAuthMethod({
				methodName,
				accessToken: testThirdPartyServiceAccessToken,
			});
			expect(thirdPartyAuthService.getUserInfos).toHaveBeenCalledWith(
				methodName,
				testThirdPartyServiceAccessToken
			);
			expect(email).toBe(testEmail);
		});
		...
		it("should throw an error if third-party auth service is not available", async () => {
			...
			await expect(
				serviceWithoutThirdParty.getEmailFromAuthMethod({
					methodName: "google",
					accessToken: testThirdPartyServiceAccessToken,
				})
